...
- [x] 회비 납부 관리
- [x] 카테고리별 통계
- [x] 예산관리 재원 흐름 DB 기반 동적 표시
- [x] 입금/출금 변경 시 예산관리 잔액 자동 동기화
...
### Phase 2.98 - 홈화면 라운드 기록 카드 고도화 (완료)
- [x] 1등 고정 표시 → 전체 참가자 랜덤 순환 (10초 간격, fade 전환)
- [x] 점수대별(60~120타) 축하/격려/응원/위로 코멘트 3개씩 랜덤 표시
- [x] 이전 라운딩 평균 대비 비교 멘트 (5단계: 크게 개선/개선/비슷/하락/크게 하락)
- [x] 선수별 멘트 중복 완전 방지 (소진 후 초기화)
- [x] 당일 경기 결과 포함 버그 수정 (< → <=)

### Phase 2.99 - 스코어·자금 통합 자동화 (완료)
- [x] 스코어 등록 시 관련 화면 통합 자동 업데이트
  - schedules.status=completed 자동 설정
  - 예약 없는 참가자 reservations confirmed 자동 생성
  - reservations 캐시 자동 갱신
- [x] 홈 라운드 카드 멘트 고도화
  - 싱글(79타 이하) 달성 시 무조건 축하 멘트
  - prevLast(직전 라운딩 스코어) 서버에서 계산 후 전달
  - 100타 이상 선수: 직전 라운딩 기준 특화 멘트 (축하/응원)
  - 멘트 우선순위: ① 싱글 축하 → ② 100타↑ 직전 기준 → ③ 평균 기준 → ④ 점수대 기본
- [x] 예산관리 재원 흐름 DB 기반 동적 계산 (하드코딩 제거)
  - 실제 incomes/expenses 기반으로 날짜별 집계
  - current_balance 역산으로 각 시점 잔액 표시
  - planned 행사·미납 회비 예정 행 자동 추가
- [x] 예산관리 총수입/총지출/현재잔액 실제 DB 기준으로 교체
- [x] 입금/출금 등록·수정·삭제 시 current_balance 자동 동기화 (syncBudgetBalance)

### Phase 3.0 - 조편성 배너 및 인터랙티브 UI (완료)
- [x] 6/13 양지파인CC 조편성 DB 등록 (1~5조, 코스·티타임·참가자)
- [x] 6/27 대영힐스CC 조편성 DB 등록 (1~6조, 청코스·미코스)
- [x] 6/13·6/27 일정 상태 open → closed (예약완료) 변경
- [x] 홈 우측 배너 '예약하기' → '조편성' 배너로 자동 전환 (closed 일정 기준)
- [x] 조편성 모달 — 탭 전환으로 다중 일정 조회 (6/13 ↔ 6/27)
- [x] 조별 코스명 뱃지(파란색) 표시
- [x] max_members 999 이상 시 인원 상한선 숨김 (홈/일정목록/예약화면)
- [x] EJS 내 require('moment') 제거 — 순수 JS 날짜 파싱으로 교체
- [x] Vercel UTC 환경 날짜 하루 빠짐 버그 수정
- [x] 전체 버튼 인터랙티브 탭 효과 일괄 적용 (13개 항목)
  - btn-outline-* / btn-secondary / btn-link / btn-light :active 추가
  - more-menu-item, 조편성 모달 버튼, info-dropdown/refresh 버튼
  - sortable 헤더, 페이지네이션, 아코디언
  - 전체 버튼 -webkit-tap-highlight-color 제거
- [x] 조편성 모달 '일정 상세 보기' 탭 효과 — a태그→button + 120ms 딜레이 이동

### 2026-06-08 데이터 정비
- [x] 예약현황 팀 선택 드롭다운 최대 4팀 → 10팀으로 확장
- [x] 6/13 양지파인CC 조편성 수정 — 멤버명(장하원→정하원) 및 팀 재배치
- [x] 자금 데이터 정확성 개선
  - 3/14 양지파인CC 묶음 지출 1건 → 그린피/식사/기타 3건으로 분리
  - 4/18 군산CC 지출 4건 설명 오기재 수정 (고향집·옥구농협·군산레져산업·하제끝집)
- [x] 입출금내역 PNG → 엑셀 변환 (3 시트: 입금내역/출금내역/입출금 반기보고)
  - 카카오뱅크 실잔액 1,121,896원과 일치 검증 완료

### Phase 3.1 - 조편성 관리 고도화 (완료)
- [x] 이미지 명단(사진) OCR 텍스트 추출 → DB 자동 등록 워크플로우 정착
- [x] 7/25 대영베이스CC 조편성 등록 (IN코스 1~3조 / OUT코스 4~6조, 24명)
- [x] 8/22 대영힐스CC 조편성 등록 (청코스 1~3조 / 미코스 4~6조, 24명)
- [x] confirmed 상태 도입 — 관리자가 직접 등록한 확정 조편성 (일반 사용자 예약 신청 차단)
- [x] confirmed/closed 상태 일정 목록·상세 화면에서 '마감' 배지로 통합 표시
- [x] 팀 배정 버튼 → 조편성표 모달로 교체
  - 황색(#fffde7) 배경 테마로 실물 명단표와 유사한 디자인
  - 조 / 코스(청·미 색상 뱃지) / 티타임 / 성명1~4 컬럼 표시
  - 팀 미배정 시 자동 팀 배정 버튼 노출, 배정 완료 시 조편성표만 표시
